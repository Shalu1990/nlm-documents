...
<sch:schema xmlns:sch="http://purl.oclc.org/dsdl/schematron" xmlns:mml="http://www.w3.org/1998/Math/MathML" queryBinding="xslt2">
  ...
  <sch:pattern>
    <sch:rule context="journal-subtitle | abbrev-journal-title | trans-title-group" role="error"><!--No other children of journal-title-group used-->
      <sch:report id="jmeta4" test="parent::journal-title-group">Unexpected use of "<sch:name/>" in "journal-title-group". "journal-title-group" should only contain "journal-title".</sch:report>
      </sch:rule>
  </sch:pattern>
  
  <sch:pattern>
    <sch:rule context="journal-meta/issn" role="error"><!--Correct attribute value inserted; ISSN matches expected syntax-->
      <sch:assert id="jmeta5a" test="@pub-type='ppub' or @pub-type='epub'">ISSN should have attribute pub-type="ppub" for print or pub-type="epub" for electronic publication.</sch:assert>
      <sch:assert id="jmeta5b" test="matches(.,'^[0-9]{4}\-[0-9]{3}([0-9]{1}|X)$')">ISSN does not conform to the expected syntax of two groups of four digits separated by a hyphen (-). The final character can be an 'X' rather than a number.</sch:assert>
    </sch:rule>
  </sch:pattern>
  
  <sch:pattern>
    <sch:rule context="journal-meta/isbn" role="error"><!--Other expected and unexpected elements-->
      <sch:report id="jmeta6" test=".">Do not use the ISBN element in journal metadata.</sch:report>
      </sch:rule>
  </sch:pattern>
    
  <sch:pattern>
    <sch:rule context="journal-meta" role="error"><!--Other expected and unexpected elements-->
      <sch:assert id="jmeta7" test="publisher">Journal metadata should include a "publisher" element.</sch:assert>
    </sch:rule>
  </sch:pattern>
  ...
  <sch:pattern><!--Has at least one subject code been included?-->
    <sch:rule  context="article-categories" role="error">
      <sch:assert id="ameta3a" test="descendant::subj-group[@subj-group-type='subjects']">Subject code(s) should be contained in article metadata section. Contact NPG Editorial Production for values that should be used.</sch:assert>
    </sch:rule>
  </sch:pattern>
  ...
  <sch:pattern><!--Is the old-style subject code allowed in this journal?-->
    <sch:rule context="subject[@content-type='subject']" role="error">
      <sch:assert id="ameta3c" test="contains(.,$journal-id) or not($journal-id=$allowed-values/journal[@title=$journal-title]/id) or not($allowed-values/journal[@title=$journal-title]) or not($allowed-values/journal[id=$journal-id])">Unexpected subject code (<sch:value-of select="."/> for <sch:value-of select="$journal-title"/>. Contact NPG Editorial Production for values that should be used.</sch:assert>
    </sch:rule>
  </sch:pattern>
