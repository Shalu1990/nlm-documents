...
<schema xmlns="http://purl.oclc.org/dsdl/schematron"
  ...
  <let name="maestro-aj"
        value="if (matches($pcode,'^(nmstr|palmstr|mtm|hortres|sdata|bdjteam|palcomms|hgv|npjbiofilms|npjschz|npjamd|micronano|npjqi|mto|npjsba|npjmgrav|celldisc|npjbcancer)$')) then 'yes'     else if ($pcode eq 'boneres' and number($volume) gt 1) then 'yes'     else if ($pcode eq 'npjpcrm' and number($volume) gt 23) then 'yes'     else ()"/>
  <let name="maestro-rj"
